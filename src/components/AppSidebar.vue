...
<template>
  <aside
    ...
    <div class="flex shrink-0 items-start gap-2" :class="sidebarExpanded ? 'flex-row' : 'flex-col items-center gap-3'">
      <RouterLink to="/site/dashboard"
      ...
      <button type="button"
        ...
        :class="sidebarExpanded ? '' : 'mx-auto'" :aria-expanded="sidebarExpanded"
        :aria-label="sidebarExpanded ? L.sidebarCollapse : L.sidebarExpand" @click="sidebarExpanded = !sidebarExpanded">
    ...
    <nav class="mt-5 min-h-0 min-w-0 flex-1 overflow-y-auto pb-4"
      ...
      <template v-if="sidebarExpanded">
        <div v-for="group in navGroups" :key="group.key" :class="group.key === 'schedule' ? '' : 'mt-4'">
          ...
          <div v-show="group.open.value" class="ml-1 space-y-0.5 border-l pl-2" :class="group.borderClass">
            <RouterLink v-for="item in group.items" :key="item.path + (item.exact ? ':exact' : '')" :to="item.path"
              class="group" :class="linkClass(item)">
      ...
      <template v-else>
        <template v-for="(group, groupIndex) in navGroups" :key="'compact-' + group.key">
          ...
          <RouterLink v-for="item in group.items" :key="'c-' + item.path + (item.exact ? ':exact' : '')" :to="item.path"
            :title="item.label" :class="linkClassCollapsed(item)">
            <component :is="item.icon" class="h-4 w-4 shrink-0"
              :class="navItemActive(item) ? 'text-flare-600' : 'text-slate-400'" />
          </RouterLink>
        </template>

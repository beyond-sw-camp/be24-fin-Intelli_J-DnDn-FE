...
const groupProgress = computed(() => {
  ...
  for (const t of aiTasks.value) {
    if (!map.has(t.group)) {
      map.set(t.group, { plan: 0, actual: 0, count: 0, started: false })
    }

    const m = map.get(t.group)
    const plans = Array.isArray(t.workPlans) ? t.workPlans : []

    // 계획 시작일 또는 실제 시작일이 오늘 이전이면 시작된 공종으로 판단
    const taskStarted = !!t.start && t.start <= today
    const workPlanStarted = plans.some((wp) => {
      const planStart = getWorkPlanStart(wp)
      return (wp.actualStart && wp.actualStart <= today) || (planStart && planStart <= today)
    })
    const hasActualProgress = plans.some((wp) => Number(wp.actualPct ?? 0) > 0)

    if (taskStarted || workPlanStarted || hasActualProgress) {
      m.started = true
    }

    m.plan += t.weight

    const actualAvg = plans.length
      ? plans.reduce((sum, wp) => sum + (wp.actualPct ?? 0), 0) / plans.length
      : 0
  ...
  return Array.from(map.entries()).map(([group, v]) => {
    ...
    const isDelayed = v.started && diff < 0
    ...
    return {
      ...
      started: v.started,
      isDelayed,
      statusLabel: !v.started ? '시작 전' : isDelayed ? '지연 위험' : '정상',
    }
  })
})
    ...
    >
      ...
      <div class="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        <div
          ...
          :class="g.isDelayed ? 'border-rose-200 bg-rose-50/20' : 'border-forena-100'"
        >
          <!-- 상태 뱃지 -->
          <span

...
import {
  ...
  ShieldCheck, UserCog, Trash2, Download, Sparkles
...
function reportsForDate(dateStr) {
  return reports.value.filter(r => r.date === dateStr && visibleProcesses.value.includes(r.process))
}
...
async function openCreate() {
  ...
  try {
    ...
    availableTodayOrders.value = orders.filter(o => 
      (o.tradeType === targetProcess || o.trade === targetProcess) && 
  ...
  } catch (e) { console.error("작업 지시서 조회 실패:", e); }
}
...
async function selectTaskForReport(order) {
  ...
  editingReport.value = {
    id: `r_${Date.now()}`, date: selectedDate.value, process: myProcess.value,
    author: '나(담당자)', location: order.location || order.title.replace(`[${myProcess.value}] `,'').replace(' 작업지시서',''), 
    workers: order.workerCount || 0, equipmentCount: 0,
    equipmentList: (order.equipments || []).map(eq => `${getEqKorName(eq.equipmentName || eq.type)} ${eq.equipmentCount || eq.count || 1}대`), 
    equipmentInput: { name: '', count: 1 }, 
    todayWork: `[지시내용]\n${order.instructionContent || ''}\n\n[실제 완료]\n`, 
    startDate: '', endDate: '', 
    tradeStartDate: '', tradeEndDate: '', // 🌟 진척률 계산용 '전체 공정 기간' 변수 추가
    progress: 0, processProgress: 0, prevProgress: 0,
    completion: '미완료', notes: '', photos: [], files: [], status: '작성 전',
    workPlanId: order.workPlanId || null, 
    tomorrowWorkPlanId: null, tomorrowLocation: order.location || '',
    tomorrowWorkers: 0, tomorrowEquipmentList: [], tomorrowEquipmentInput: { name: '', count: 1 },
    tomorrowPlan: '', tomorrowSafety: '',
  };

  try {
    // 1. 전체 공정 기간(철근 전체)을 구하기 위해 모든 계획을 불러옵니다.
    ...
    const toDateString = (d) => Array.isArray(d) ? `${d[0]}-${String(d[1]).padStart(2,'0')}-${String(d[2]).padStart(2,'0')}` : d;

    let tradeMinStart = '9999-12-31';
    let tradeMaxEnd = '0000-01-01';

    // 🌟 2. 내 공정(철근)의 모든 계획을 뒤져서 '진짜 시작일'과 '진짜 종료일'을 찾습니다.
    allPlans.forEach(p => {
      if (p.trade === myProcess.value || p.tradeType === myProcess.value) {
    ...
    if (order.workPlanId) {
      const currentPlan = allPlans.find(p => p.idx === order.workPlanId || p.id === order.workPlanId);
      if (currentPlan) {
    ...
    for (let i = 1; i <= 3; i++) {
      try {
        ...
        const pTarget = pArr.find(r => (r.tradeType === myProcess.value || r.process === myProcess.value) && r.workPlanId === order.workPlanId);
    ...
    availableTomorrowPlans.value = allPlans.filter(p => 
      (p.trade === myProcess.value || p.tradeType === myProcess.value) &&
      tmrwStr >= toDateString(p.startDate) && tmrwStr <= toDateString(p.endDate)
    );
  } catch(e) { console.error("명일 주간계획 로드 실패", e); }
...
async function openEditor(report) {
  ...
  editingReport.value = {
    ...
    equipmentInput: { name: '', count: 1 },
    tradeStartDate: '', tradeEndDate: '', // 🌟 추가
    tomorrowWorkPlanId: null, tomorrowLocation: report.location || '',
    tomorrowWorkers: 0, tomorrowEquipmentList: [], tomorrowEquipmentInput: { name: '', count: 1 },
    tomorrowPlan: report.tomorrowPlan || '', tomorrowSafety: '',
  ...
  try {
    ...
    if (report.workPlanId && (!editingReport.value.startDate || !editingReport.value.endDate)) {
      const currentPlan = allPlans.find(p => p.idx === report.workPlanId || p.id === report.workPlanId);
      if (currentPlan) {
         editingReport.value.startDate = toDateString(currentPlan.startDate) || '';
         editingReport.value.endDate = toDateString(currentPlan.endDate) || '';
      }
...
// feat : 공정 전체 기간 비례 금일 증가분(%) 자동 계산 로직
const calcInfo = computed(() => {
  const r = editingReport.value
  if (!r) return null
  
  // 🌟 핵심: 진척률 계산은 입력칸의 '세부 일정'이 아니라 숨겨둔 '전체 공정 기간'을 기준으로 합니다!
...
watch(
  () => [editingReport.value?.progress, editingReport.value?.startDate, editingReport.value?.endDate, editingReport.value?.prevProgress],
  () => {
    const r = editingReport.value;
    if (!r) return;
    const info = calcInfo.value;
    if (!info) {
      r.processProgress = r.prevProgress || 0; return;
    }
...
function removeEquipment(idx) { editingReport.value.equipmentList.splice(idx, 1); }
...
function removeTomorrowEquipment(idx) { editingReport.value.tomorrowEquipmentList.splice(idx, 1); }
...
async function submitReport() {
  ...
  const payload = {
    ...
    tomorrowEquipments: r.tomorrowEquipmentList.map(eqStr => {
       ...
       return match ? { type: match[1].trim(), count: parseInt(match[2], 10) } : { type: '기타', count: 1 };
    })
  };
...
function openViewer(report) { viewingReport.value = report; }
  ...
  <div class="flex h-full min-h-0 flex-col gap-4 pb-6">
    ...
    <div class="flex shrink-0 flex-wrap items-center gap-3 rounded-xl border border-forena-100 bg-white px-4 py-3">
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        <button v-if="currentRole === ROLES.WORKER" @click="openCreate" class="inline-flex items-center gap-1.5 rounded-lg bg-flare-500 px-3 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-flare-600">
    ...
    <div class="min-h-0 flex-1 overflow-y-auto">
      ...
      <div v-if="activeTab === 'today'" class="grid grid-cols-1 gap-3 lg:grid-cols-2 mt-2">
        <div v-if="todayReports.length === 0" class="col-span-1 lg:col-span-2 flex flex-col items-center justify-center gap-3 py-16 text-center">
           <FileText class="h-10 w-10 text-slate-300" />
           <p class="text-sm text-slate-500">작성된 공사일보가 없습니다.</p>
           <button v-if="currentRole === ROLES.WORKER" @click="openCreate" class="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-flare-500 px-4 py-2 text-xs font-bold text-white hover:bg-flare-600">
             <Plus class="h-3.5 w-3.5" /> 작성하기
           </button>
        </div>

        <div v-for="r in todayReports" :key="r.id" class="flex flex-col rounded-xl border bg-white border-forena-100 hover:border-flare-200 hover:shadow-sm transition">
          <div class="flex items-start justify-between gap-3 border-b border-forena-100 px-4 py-3">
            ...
            <div class="flex shrink-0 gap-1">
              <button @click="openViewer(r)" class="rounded-md border border-forena-200 bg-white p-1.5 text-forena-600 hover:bg-forena-50" title="상세 보기"><Eye class="h-3.5 w-3.5" /></button>
        ...
        <div class="flex max-h-[80vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
          <div class="flex shrink-0 items-center justify-between border-b border-forena-100 px-6 py-4">
            <div>
              ...
              <p class="mt-0.5 text-xs text-forena-500">{{ fmtKor(selectedDate) }} 기준 승인된 작업 지시서 목록입니다.</p>
            </div>
          ...
          <div class="overflow-y-auto px-6 py-4">
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div v-for="order in availableTodayOrders" :key="order.idx" @click="selectTaskForReport(order)"
                ...
                <div class="flex items-start justify-between mb-2">
                  <span class="rounded bg-forena-100 px-1.5 py-0.5 text-[10px] font-bold text-forena-700">{{ order.location || order.title }}</span>
                </div>
                <p class="mt-2 text-xs text-forena-500 line-clamp-2">{{ order.instructionContent || '세부 지시 내용 없음' }}</p>
                <div class="mt-3 flex items-center gap-3 text-[11px] font-semibold text-forena-600">
                  <span class="flex items-center gap-1"><Users class="h-3.5 w-3.5" /> {{ order.workerCount || 0 }}명</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
          ...
          <div class="min-h-0 flex-1 overflow-y-auto px-6 py-4">
            ...
            <div v-if="modalTab === 'today'" class="space-y-4">
              ...
              <div class="rounded-lg border border-flare-100 bg-flare-50/40 p-3">
                ...
                <p v-if="calcInfo" class="mt-1.5 text-[10px] text-forena-500">
                  <span class="block mb-0.5 text-forena-600">
                     <span class="font-bold">산식:</span> 하루 목표 <span class="font-bold">{{ calcInfo.dailyAllocation }}%</span> ÷ 금일 세부작업 <span class="font-bold">{{ calcInfo.tasksTodayCount }}건</span> = 건당 최대 <span class="font-bold">{{ calcInfo.weightPerTask }}%</span> 반영
                  </span>
              ...
              <div class="mt-5 rounded-xl border border-forena-100 bg-forena-50/30 p-3.5">
                <div class="mb-2 flex items-center justify-between">
                  <p class="flex items-center gap-1.5 text-[11px] font-bold text-forena-700">
                    <ImageIcon class="h-3.5 w-3.5 text-flare-600" /> 현장 / 도면 사진 ({{ editingReport.photos.length }})
                  </p>
                  <button @click="pickPhotos" class="inline-flex items-center gap-1 rounded-md border border-flare-200 bg-flare-50 px-2.5 py-1 text-[11px] font-bold text-flare-700 hover:bg-flare-100">
                    <Plus class="h-3 w-3" /> 사진 추가
                  </button>
                  <input ref="photoInputRef" type="file" class="sr-only" accept="image/*" multiple @change="onPhotoChange" />
                </div>
                <div v-if="editingReport.photos.length" class="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
                  <div v-for="(photo, i) in editingReport.photos" :key="photo.id" class="group relative aspect-square overflow-hidden rounded-lg border border-forena-200 bg-white">
              ...
              <div class="mt-3 rounded-xl border border-forena-100 bg-forena-50/30 p-3.5">
                <div class="mb-2 flex items-center justify-between">
                  <p class="flex items-center gap-1.5 text-[11px] font-bold text-forena-700">
                    <Paperclip class="h-3.5 w-3.5 text-flare-600" /> 첨부 파일 ({{ editingReport.files.length }})
                  </p>
                  <button @click="pickFiles" class="inline-flex items-center gap-1 rounded-md border border-flare-200 bg-flare-50 px-2.5 py-1 text-[11px] font-bold text-flare-700 hover:bg-flare-100">
                ...
                <ul v-if="editingReport.files.length" class="space-y-1.5">
                  <li v-for="(f, i) in editingReport.files" :key="f.id" class="flex items-center gap-2 rounded-lg bg-white px-2.5 py-2 ring-1 ring-forena-100">
            ...
            <div v-else-if="modalTab === 'tomorrow'" class="space-y-4 mt-2">
              <div class="rounded-xl border border-flare-200 bg-flare-50/40 p-4 mb-4">
                <label class="mb-1.5 block text-xs font-bold text-flare-800"><Sparkles class="inline h-3.5 w-3.5 text-flare-600" /> 대상 주간계획 선택</label>
                <select v-model="editingReport.tomorrowWorkPlanId" @change="onTomorrowPlanSelect" class="w-full rounded-md border border-forena-200 bg-white px-3 py-2 text-sm font-semibold text-forena-800 outline-none focus:border-flare-400">
                  <option :value="null">-- 내일 진행할 주간계획 일정을 선택하세요 --</option>
                  <option v-for="plan in availableTomorrowPlans" :key="plan.idx || plan.id" :value="plan.idx || plan.id">
                    [{{ plan.location }}] {{ plan.name }}
                  </option>
                </select>
                <p class="mt-2 text-[11px] text-forena-600">위에서 내일 일정을 선택하면 기존 데이터가 불려오며, 수정한 내용은 원본 주간계획에 덮어쓰기 됩니다.</p>
              </div>

              <div v-if="editingReport.tomorrowWorkPlanId" class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div class="sm:col-span-2">
                  <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500"><MapPin class="mr-0.5 inline h-3 w-3" />작업 위치</label>
                  <input v-model="editingReport.tomorrowLocation" class="w-full rounded-md border border-forena-200 bg-white px-2.5 py-1.5 text-xs outline-none focus:border-flare-400" />
                </div>

                <div>
                  <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500"><Clock class="mr-0.5 inline h-3 w-3" />작업 시간</label>
                  <input value="07:00 ~ 17:00" disabled class="w-full rounded-md border border-forena-200 bg-slate-50 px-2.5 py-1.5 text-xs tabular-nums text-slate-500 outline-none" />
                </div>

                <div>
                  <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500"><Users class="mr-0.5 inline h-3 w-3" />필요 인원</label>
                ...
                <div class="sm:col-span-2">
                  <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500">작업 상세 내역 <span class="text-rose-500">*</span></label>
                  <textarea v-model="editingReport.tomorrowPlan" rows="5" class="w-full resize-none rounded-md border border-forena-200 bg-white px-2.5 py-1.5 text-xs leading-relaxed outline-none focus:border-flare-400"></textarea>
                </div>
                
                <div class="sm:col-span-2">
                  <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500">안전 유의사항</label>
                  <textarea v-model="editingReport.tomorrowSafety" rows="3" placeholder="추락/낙하/화재 등 위험요인 사전 점검" class="w-full resize-none rounded-md border border-forena-200 bg-white px-2.5 py-1.5 text-xs leading-relaxed outline-none focus:border-flare-400"></textarea>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex shrink-0 items-center justify-between gap-2 border-t border-forena-100 bg-forena-50/40 px-6 py-3">
            ...
            <div class="flex gap-2">
              ...
              <button @click="submitReport" class="inline-flex items-center gap-1.5 rounded-lg bg-flare-500 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-flare-600"><Send class="h-3.5 w-3.5" /> 공사일보 제출</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="viewingReport"
        ...
        <div class="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
          ...
          <div class="min-h-0 flex-1 overflow-y-auto px-6 py-4">
            ...
            <div v-if="viewingReport.photos?.length" class="mt-3 rounded-xl border border-forena-100 px-4 py-3">
              ...
              <div class="mt-2 grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
                <div v-for="photo in viewingReport.photos" :key="photo.id" class="aspect-square overflow-hidden rounded-lg border border-forena-200 bg-white">
                  <img :src="photo.dataUrl" :alt="photo.name" class="h-full w-full object-cover" />
                </div>
              </div>
            </div>
            
            <div v-if="viewingReport.files?.length" class="mt-3 rounded-xl border border-forena-100 px-4 py-3">
              <p class="text-[10px] font-bold uppercase tracking-wide text-forena-500">관련 문서</p>
              <ul class="mt-1.5 space-y-1">
                <li v-for="f in viewingReport.files" :key="f.id" class="flex items-center gap-2 text-xs text-forena-800">

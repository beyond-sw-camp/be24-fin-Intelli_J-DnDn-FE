...
// feat: gateIdx → gate name 맵 (백엔드에서 resolve 없이 프론트 gates 배열 활용)
const gateNameByIdx = computed(() => {
  const map = {}
...
// feat: 게이트별 투입 장비 대수 (마커 배지용)
const equipCountByGateIdx = computed(() => {
  const map = {}
  for (const eq of todayEquipments.value) {
    if (eq.gateIdx != null) {
      map[eq.gateIdx] = (map[eq.gateIdx] || 0) + (eq.equipmentCount ?? 1)
    }
  }
  return map
})
...
const getEquipStatusClass = (statusLabel) => {
  switch (statusLabel) {
    case '작업중': return 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200'
    case '대기': return 'bg-amber-50 text-amber-700 ring-1 ring-amber-200'
    case '입차예정': return 'bg-violet-50 text-violet-700 ring-1 ring-violet-200'
...
const loadTodayEquipments = async () => {
  ...
  try {
    ...
    const res = await getGateEquipments(dateStr)
    // interceptor가 body.data를 반환 → 이미 배열
  ...
  if (centerRatio) {
    nextTick(() => {
      if (roundedZoom <= 1.01) {
        ...
        // feat: 줌아웃 시에도 세로 중앙 정렬 유지 (scrollTop=0 제거)
...
function zoomBlueprint(delta) { setBlueprintZoom(blueprintZoom.value + delta) }
function resetBlueprintZoom() { setBlueprintZoom(1); centerBlueprintView() }
...
function centerBlueprintView() {
  nextTick(() => {
    ...
    if (!viewport) return
    viewport.scrollLeft = Math.max(0, (viewport.scrollWidth - viewport.clientWidth) / 2)
    // FIXME: 비줌 상태에서도 세로 중앙 정렬 — overflow-y-hidden이어도 JS scrollTop 적용됨
    viewport.scrollTop = Math.max(0, (viewport.scrollHeight - viewport.clientHeight) / 2)
  })
}
...
function startBlueprintPan(event) {
  ...
  blueprintPanStart.value = { x: event.clientX, y: event.clientY, scrollLeft: viewport.scrollLeft, scrollTop: viewport.scrollTop }
...
function resetBlueprint() {
  ...
  try { window.localStorage.removeItem(BLUEPRINT_STORAGE_KEY) } catch (error) { console.error(error) }
}
...
const updateManpower = async (delta) => {
  ...
  if (next < 2) return
  try { await updateGateManpower(selectedGate.value.idx, next); await refreshGate(selectedGate.value.idx) }
  catch (error) { console.error('배치 인원 수정 실패', error) }
}

const updateVehicles = async (delta) => {
  if (!selectedGate.value) return
  const next = Math.max(0, selectedGate.value.vehicles + delta)
  try { await updateGateVehicles(selectedGate.value.idx, next); await refreshGate(selectedGate.value.idx) }
  catch (error) { console.error('진입 차량 수정 실패', error) }
}

const toggleMachine = async (machineId) => {
  if (!selectedGate.value) return
  try { await toggleGateMachine(selectedGate.value.idx, machineId); await refreshGate(selectedGate.value.idx) }
  catch (error) { console.error('세척 기계 토글 실패', error) }
}

const addMachine = async () => {
  if (!selectedGate.value) return
  try { await addGateMachine(selectedGate.value.idx); await refreshGate(selectedGate.value.idx) }
  catch (error) { console.error('세척 기계 추가 실패', error) }
}

const removeMachine = async (machineId) => {
  if (!selectedGate.value) return
  try { await removeGateMachine(selectedGate.value.idx, machineId); await refreshGate(selectedGate.value.idx) }
  catch (error) { console.error('세척 기계 삭제 실패', error) }
}
...
const addCustomGate = async (event) => {
  ...
  try {
    const newIdx = await createGate({ name: `Gate ${nextSeq}`, x: Math.max(0, Math.min(100, x)), y: Math.max(0, Math.min(100, y)) })
    await loadGates()
    selectedGateId.value = newIdx
    isAddMode.value = false
  } catch (error) { console.error('게이트 등록 실패', error) }
}
...
const removeGate = async (gateId) => {
  try {
    ...
    if (selectedGateId.value === gateId) selectedGateId.value = gates.value[0]?.idx ?? null
  } catch (error) { console.error('게이트 삭제 실패', error) }
}
...
const onMarkerDragStart = (gate, event) => {
  ...
  if (event.dataTransfer) { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', String(gate.idx)) }
}
...
const onMarkerDragEnd = async (gate, event) => {
  const point = event.clientX === 0 && event.clientY === 0 ? lastDragPoint.value : { x: event.clientX, y: event.clientY }
  if (dragMoved.value && mapRef.value) {
    const rect = mapRef.value.getBoundingClientRect()
    const nextX = clampPercent(((point.x - rect.left) / rect.width) * 100)
    const nextY = clampPercent(((point.y - rect.top) / rect.height) * 100)
    gate.x = nextX; gate.y = nextY
    try { await updateGatePosition(gate.idx, { x: nextX, y: nextY }) }
    catch (error) { console.error('게이트 좌표 저장 실패', error); await refreshGate(gate.idx) }
  }
  if (dragMoved.value) suppressClickGateId.value = draggingGateId.value
...
const onGateClick = (gateId, event) => {
  event.stopPropagation()
  if (suppressClickGateId.value === gateId) { suppressClickGateId.value = null; return }
...
<template>
  <div class="space-y-6 pb-10">
    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <!-- 도면 -->
      <div class="lg:col-span-2 relative overflow-visible rounded-3xl border border-forena-100 bg-white p-3 shadow-card">
        <div class="absolute right-4 top-4 z-30 flex items-center gap-2">
          <button type="button"
            ...
            <MapIcon class="h-4 w-4" />게이트 추가 모드
          </button>
        ...
        <div class="absolute bottom-6 right-6 z-40 flex items-center gap-1 rounded-lg border border-forena-100 bg-white/95 p-1 text-[10px] font-bold text-forena-700 shadow-sm backdrop-blur">
          <button type="button" class="flex h-6 w-6 items-center justify-center rounded-md hover:bg-forena-50" title="도면 축소" @click.stop="zoomBlueprint(-BLUEPRINT_ZOOM_STEP)">-</button>
          <button type="button" class="min-w-[42px] rounded-md px-1.5 py-0.5 hover:bg-forena-50" title="기본 배율로 초기화" @click.stop="resetBlueprintZoom">{{ blueprintZoomPercent }}</button>
          <button type="button" class="flex h-6 w-6 items-center justify-center rounded-md hover:bg-forena-50" title="도면 확대" @click.stop="zoomBlueprint(BLUEPRINT_ZOOM_STEP)">+</button>
          <button type="button" class="inline-flex h-6 items-center gap-1 rounded-md border border-forena-100 px-1.5 text-[10px] font-black text-forena-700 hover:bg-forena-50" title="도면 배율 원복" @click.stop="resetBlueprintZoom">
            <RotateCcw class="h-3 w-3" />원복
          </button>
        </div>

        <div ref="mapViewportRef"
          class="relative h-[58vw] min-h-[520px] max-h-[650px] rounded-2xl bg-slate-100 p-2 select-none"
          :class="[isBlueprintZoomed ? 'overflow-auto' : 'overflow-x-auto overflow-y-hidden', isPanningBlueprint ? 'cursor-grabbing' : isAddMode ? 'cursor-crosshair' : isBlueprintZoomed ? 'cursor-grab' : 'cursor-default']"
          @wheel="onBlueprintWheel" @pointerdown="startBlueprintPan" @pointermove="moveBlueprintPan"
          @pointerup="stopBlueprintPan" @pointerleave="stopBlueprintPan">
          <div class="relative flex min-h-full min-w-full" :class="isBlueprintZoomed ? 'items-start justify-start' : 'items-center justify-center'">
            <div ref="mapRef" class="relative min-w-[560px] overflow-hidden rounded-xl bg-white shadow-inner"
              ...
              :style="{ aspectRatio: blueprintAspectRatio, width: `${blueprintZoom * 100}%` }"
              @click="isAddMode && addCustomGate($event)" @dragover="onMapDragOver">
              <img :src="activeBlueprint" alt="공사현장 도면" class="absolute inset-0 h-full w-full select-none object-fill" draggable="false" @load="updateBlueprintAspect" />

              <button v-for="gate in gates" :key="gate.idx" data-gate-marker
                class="absolute flex flex-col items-center gap-1 transition-all"
                :class="draggingGateId === gate.idx ? 'cursor-grabbing opacity-50' : 'cursor-grab'"
                :style="{ left: gate.x + '%', top: gate.y + '%', transform: `translate(-50%, -50%) scale(${draggingGateId === gate.idx ? gateMarkerScale * 1.08 : gateMarkerScale})`, zIndex: draggingGateId === gate.idx ? 50 : 10 }"
                :draggable="!isAddMode"
                @dragstart="onMarkerDragStart(gate, $event)" @drag="onMarkerDrag($event)"
                @dragend="onMarkerDragEnd(gate, $event)" @click="onGateClick(gate.idx, $event)">
                <div class="relative flex h-8 w-8 items-center justify-center rounded-full border-[3px] border-white text-white shadow-xl transition-colors sm:h-9 sm:w-9 xl:h-10 xl:w-10 xl:border-4"
                  ...
                  <AlertCircle v-if="gate.inefficient" class="absolute -right-2 -top-2 h-4 w-4 rounded-full fill-amber-500 text-white sm:h-5 sm:w-5" />
                </div>
                <div class="flex flex-col items-center gap-0.5 rounded-lg border border-forena-100 bg-white/90 px-1.5 py-0.5 text-[9px] font-bold shadow-sm sm:px-2 sm:py-1 sm:text-[10px]">
                  <!-- feat: 실제 투입 장비 대수 (equipCountByGateIdx) 우선, 없으면 vehicles -->
                  <span class="whitespace-nowrap">
                    {{ gate.name }} ({{ equipCountByGateIdx[gate.idx] ?? gate.vehicles ?? 0 }}대)
                  </span>
                </div>
              </button>
            </div>
      ...
      <div class="space-y-4 lg:flex lg:h-[58vw] lg:min-h-[520px] lg:max-h-[650px] lg:flex-col">
          ...
          <div class="flex flex-col gap-3 border-b border-forena-100 bg-forena-50/50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            ...
            <div class="flex flex-wrap items-center justify-end gap-2">
              <div class="flex items-center gap-1 rounded-full bg-amber-100 px-1.5 py-1 sm:gap-1.5">
                <button type="button" class="flex h-7 w-7 items-center justify-center rounded-full bg-white text-sm font-black text-amber-800 shadow-sm transition hover:scale-105 hover:bg-amber-50" @click="updateVehicles(-1)">-</button>
                <span class="px-1 text-xs font-bold text-amber-800 sm:text-sm">대기 {{ selectedGate.vehicles }}대</span>
                <button type="button" class="flex h-7 w-7 items-center justify-center rounded-full bg-white text-sm font-black text-amber-800 shadow-sm transition hover:scale-105 hover:bg-amber-50" @click="updateVehicles(1)">+</button>
              </div>
              <span class="rounded-full border px-2.5 py-1 text-xs font-bold sm:px-3" :class="getStatusColor(selectedGate)">{{ selectedGate.congestionLabel }}</span>
              <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full text-rose-400 transition hover:bg-rose-50 hover:text-rose-600" title="게이트 삭제" @click="removeGate(selectedGate.idx)">
                <Trash2 class="h-4 w-4 sm:h-5 sm:w-5" />
              </button>
              <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full text-forena-400 transition hover:bg-forena-100 hover:text-forena-600" @click="selectedGateId = null">
          ...
          <div class="space-y-7 overflow-y-auto p-5 sm:p-6 lg:flex-1">
            <div>
              ...
              <div class="mt-2 h-3 overflow-hidden rounded-full bg-slate-100">
                <div class="h-full transition-all duration-500" :class="getMarkerColor(selectedGate)" :style="{ width: Math.min((selectedGate.vehicles / 20) * 100, 100) + '%' }"></div>
              </div>
            </div>
            ...
            <div>
              <div class="flex items-center justify-between">
                <h3 class="flex items-center gap-2 text-sm font-bold text-forena-900">
                  <Settings2 class="h-4 w-4 text-flare-600" />세척 설비 가동 ({{ selectedGate.machines.length }}대)
                </h3>
                <button type="button" class="bg-slate-100 text-slate-600 rounded-lg p-1 hover:bg-slate-200" @click="addMachine">
                  <span class="text-sm font-bold leading-none">+</span>
                </button>
              </div>
              <div v-if="selectedGate.machines.length === 0" class="mt-3 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-800">
            ...
            <div>
              <h3 class="flex items-center gap-2 text-sm font-bold text-forena-900">
                <Users class="h-4 w-4 text-flare-600" />게이트 배치 인원
              </h3>
              <div class="mt-3 flex items-center justify-between rounded-2xl border border-forena-100 bg-forena-50/30 p-2">
                <button class="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-lg font-bold shadow-sm hover:bg-slate-50 sm:h-10 sm:w-10 sm:text-xl" @click="updateManpower(-1)">-</button>
            ...
            <div>
              ...
              <div v-if="recommendedGate" class="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-blue-800 shadow-sm">
                <p class="text-sm font-semibold">현재 게이트가 매우 혼잡합니다.</p>
                <p class="mt-2 text-sm font-bold text-blue-900">가장 가까운 우회 경로: {{ recommendedGate.name }} (현재 진입: {{ recommendedGate.vehicles }}대)</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="!isLoading && gates.length === 0"
          ...
          <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-flare-50 text-flare-600"><MapIcon class="h-6 w-6" /></span>
          <p class="text-sm font-bold text-forena-900">아직 등록된 게이트가 없습니다.</p>
          <p class="text-xs text-forena-500">우측 상단 <span class="font-semibold text-flare-600">'게이트 추가 모드'</span>를 눌러 도면 위에 게이트를 배치하세요.</p>
        </div>
        ...
        <div v-else-if="gates.length > 0"
          ...
          <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-forena-50 text-forena-600"><Navigation class="h-6 w-6" /></span>
          <p class="text-sm font-bold text-forena-900">게이트를 선택해주세요</p>
          <p class="text-xs text-forena-500">도면의 마커를 클릭하면 상세 정보가 표시됩니다.</p>
        </div>

        <div v-else class="flex min-h-[400px] items-center justify-center rounded-3xl border border-forena-100 bg-white/60 text-sm text-forena-500 shadow-card">
          게이트 정보를 불러오는 중...
        </div>
      </div>
    </div>
    ...
    <div class="w-full rounded-3xl border border-forena-100 bg-white p-6 shadow-card ring-1 ring-forena-50">
      <div class="flex flex-col gap-3 border-b border-forena-50 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-forena-500">작업 지시서 장비 데이터 연동 목록</p>
          </div>
        </div>
        <button type="button"
          ...
          @click="loadTodayEquipments">
          <RefreshCw class="h-4 w-4" :class="{ 'animate-spin': isEquipLoading }" />새로고침
        </button>
      </div>
      ...
      <div v-else-if="todayEquipments.length === 0" class="mt-6 flex flex-col items-center justify-center py-10 gap-2 text-sm text-forena-500">
        <Truck class="h-8 w-8 text-forena-200" />
        <p>금일 배정된 투입 장비가 없습니다.</p>
        <p class="text-xs">작업 지시서에 장비를 등록하면 여기에 표시됩니다.</p>
      </div>
      ...
      <div v-else class="mt-6 overflow-x-auto">
        <table class="min-w-full divide-y divide-forena-100 text-sm">
          ...
          <tbody class="divide-y divide-forena-50">
            <tr v-for="equipment in todayEquipments"
              :key="`${equipment.workOrderIdx}-${equipment.gateIdx}-${equipment.equipmentName}`"
              class="transition-colors hover:bg-slate-50/80">
              <td class="px-4 py-4">
                <div class="font-bold text-forena-900">{{ equipment.equipmentName }}</div>
                <div class="mt-1 text-xs font-medium text-forena-500">{{ equipment.equipmentType }}</div>
              </td>
              <td class="px-4 py-4 font-medium text-forena-700">
                <!-- feat: gateNameByIdx computed로 gates 배열에서 게이트명 resolve -->
                {{ equipment.gateIdx != null ? (gateNameByIdx[equipment.gateIdx] ?? `Gate ${equipment.gateIdx}`) : '미배정' }}
              </td>
              <td class="px-4 py-4 font-medium text-forena-700">{{ equipment.equipmentCount ?? 1 }}대</td>
              <td class="px-4 py-4">
                <span class="font-semibold text-sky-700">{{ equipment.workOrderRef }}</span>
              </td>
              <td class="px-4 py-4">
                <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-bold" :class="getEquipStatusClass(equipment.statusLabel)">
                  {{ equipment.statusLabel || '대기' }}
                </span>
              </td>
            </tr>
          </tbody>

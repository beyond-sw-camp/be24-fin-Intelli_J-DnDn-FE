...
import {
  CalendarDays, ChevronLeft, ChevronRight, FileText, Plus, Save, Send, Eye, Pencil,
  Users, Wrench, MapPin, ClipboardList, CheckCircle2, AlertTriangle, Clock,
  ShieldCheck, UserCog, Trash2, X, Sparkles, RefreshCw, Truck, DoorOpen,
  Image as ImageIcon, Paperclip, Search, ArrowRightLeft
} from 'lucide-vue-next'

import {
  createWorkOrder, getWorkOrderList, updateWorkOrder, approveWorkOrder
} from '@/api/workOrder'

const EQUIPMENT_ENUM_MAP = {
  굴삭기: 'EXCAVATOR', 미니굴삭기: 'MINI_EXCAVATOR', 백호: 'BACKHOE_LOADER', 드래그라인: 'DRAGLINE_EXCAVATOR',
  덤프트럭: 'DUMP_TRUCK', '트럭 믹서': 'CONCRETE_MIXER_TRUCK', 트랙터: 'TRACTOR', 트레일러: 'TRAILER', 스크레이퍼: 'SCRAPER',
  타워크레인: 'TOWER_CRANE', '모바일 크레인': 'MOBILE_CRANE', '크롤러 크레인': 'CRAWLER_CRANE', 지게차: 'FORKLIFT', 리프트: 'CONSTRUCTION_HOIST',
  불도저: 'BULLDOZER', '모터 그레이더': 'MOTOR_GRADER', 롤러: 'ROAD_ROLLER', 콤팩터: 'PLATE_COMPACTOR',
  '아스팔트 피니셔': 'ASPHALT_PAVER', '밀링 머신': 'MILLING_MACHINE', 살수차: 'WATER_TRUCK', '노면 절단기': 'CONCRETE_CUTTER',
  '파일 드라이버': 'PILE_DRIVER', 보링머신: 'BORING_MACHINE', 어스오거: 'EARTH_AUGER', RCD: 'REVERSE_CIRCULATION_DRILL',
  '콘크리트 펌프카': 'CONCRETE_PUMP_TRUCK', '배치 플랜트': 'BATCHING_PLANT', 바이브레이터: 'CONCRETE_VIBRATOR',
  브레이커: 'HYDRAULIC_BREAKER', 니블러: 'NIBBLER', 크러셔: 'CRUSHER', 고소작업차: 'AERIAL_WORK_PLATFORM', TBM: 'TUNNEL_BORING_MACHINE',
}
...
const visibleTabs = computed(() => {
  if (currentRole.value === ROLES.WORKER) return [{ key: myProcess.value, label: `${myProcess.value} 공정` }]
...
const filteredOrders = computed(() => {
  ...
  if (searchKeyword.value.trim()) {
    ...
    r = r.filter(o => o.location.toLowerCase().includes(k) || o.workDetail.toLowerCase().includes(k) || o.id.toLowerCase().includes(k))
  }
  return r
})
...
const missingProcesses = computed(() => {
  ...
  const existing = new Set(workOrders.value.filter((o) => o.date === filterDate.value).map((o) => o.process))
...
const gateAssignment = computed(() => {
  ...
  workOrders.value.filter((o) => o.date === filterDate.value).forEach((o) => {
  ...
  return GATES.value.map((g) => {
    ...
    return { ...g, in: a.in, out: a.out, total, ratio: Math.min(ratio, 1.5), level, name: g.gateName, desc: '' }
  })
})
...
async function openCreate() {
  ...
  try {
    ...
    availableTasks.value = plans.filter(p => {
      const matchTrade = (p.trade === targetProcess || p.tradeType === targetProcess)
      const matchDate = targetDate >= toDateString(p.startDate) && targetDate <= toDateString(p.endDate)
      return matchTrade && matchDate
    })

    if (availableTasks.value.length === 0) {
      alert(`${targetDate} 일자에 등록된 [${targetProcess}] 세부 작업이 없습니다. 주간계획서를 먼저 작성해주세요.`)
      return
    }
...
function addEquipment() {
  ...
  editing.value.equipment.push({
    id: `eq_${Date.now()}`, name: inp.name.trim(), count: inp.count, gateIn: inp.gateIn, gateOut: inp.gateOut,
  })
...
function onPhotoChange(e) {
  Array.from(e.target.files || []).forEach((f) => {
    ...
    reader.onload = (ev) => editing.value.photos.push({
...
function onFileChangeInput(e) {
  Array.from(e.target.files || []).forEach((f) =>
    editing.value.files.push({ id: `f_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`, name: f.name, size: f.size, type: f.type })
  )
  e.target.value = ''
}
...
async function submitOrder() {
  ...
  const equipmentList = r.equipment.map((eq) => {
    const safeGateIdx = typeof eq.gateIn === 'string' ? parseInt(eq.gateIn.replace(/\D/g, ''), 10) : eq.gateIn
    return {
...
function persist(newStatus, historyLabel) {
  ...
  r.status = newStatus
  r.history.push({ at: new Date().toISOString().slice(0, 16).replace('T', ' '), who: r.author, what: historyLabel })
...
function gateName(id) { return GATES.value.find((g) => g.gateIdx === id)?.gateName || id }
...
async function fetchWorkOrders() {
  try {
    ...
    let serverData = Array.isArray(response) ? response : response?.data?.data || response?.data || []
    if (!Array.isArray(serverData)) return

    workOrders.value = serverData.map((dto) => {
      let locationStr = dto.title ? dto.title.replace(`[${dto.tradeType}] `, '').replace(' 작업지시서', '') : ''
      ...
      return {
        id: `WO-${dto.idx}`, date: dto.dueDate, process: dto.tradeType, partner: '한울중기', location: locationStr,
        workTime: time, workers: dto.workerCount || 0, workDetail: detail, safety: safetyText, notes: '',
        status: dto.statusCode === 'APPROVED' ? '승인 완료' : dto.statusCode === 'REJECTED' ? '반려' : dto.statusCode === 'OPEN' ? '승인 대기' : '작성 전',
        author: '작성자', createdAt: '-', files: [], photos: [], history: [],
        equipment: (dto.equipments || []).map((eq) => ({
          id: `eq_${eq.idx}`, name: ENUM_TO_KOR_MAP[eq.equipmentName] || eq.equipmentName, count: eq.equipmentCount,
          gateIn: `G${eq.gateIdx}`, gateOut: `G${eq.gateIdx}`,
        })),
      }
    })
  } catch (e) {
  ...
  <div class="flex h-full min-h-0 flex-col gap-4 pb-6">
    <div class="flex shrink-0 flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <div class="flex overflow-hidden rounded-lg border border-forena-200 bg-white">
          <button class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold transition"
        ...
        <button @click="openCreate" class="inline-flex items-center gap-1.5 rounded-lg bg-flare-500 px-3 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-flare-600">
          <Plus class="h-3.5 w-3.5" /> 작업 지시서 작성
        </button>
      </div>
    </div>

    <div v-if="currentRole === ROLES.WORKER" class="flex shrink-0 items-start gap-2.5 rounded-xl border border-flare-200 bg-flare-50/60 px-4 py-3">
      <UserCog class="mt-0.5 h-4 w-4 shrink-0 text-flare-600" />
      <p class="text-xs text-forena-800">
        현재 계정은 <span class="font-bold text-flare-700">{{ myProcess }} 공정 담당자</span>입니다. {{ myProcess }} 공정 작업 지시서만 조회 · 작성 · 수정할 수 있습니다.
      </p>
    </div>
    <div v-else class="flex shrink-0 items-start gap-2.5 rounded-xl border border-emerald-200 bg-emerald-50/60 px-4 py-3">
      <ShieldCheck class="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />
      <p class="text-xs text-forena-800">
        현재 계정은 <span class="font-bold text-emerald-700">현장 총 관리자</span>입니다. 전체 공정 작업 지시서를 조회하고 승인/반려할 수 있습니다.
        <span v-if="missingProcesses.length" class="ml-1 font-bold text-rose-600">· 미작성 공정: {{ missingProcesses.join(', ') }}</span>
      </p>
    </div>
    ...
    <div class="flex shrink-0 flex-wrap items-center gap-3 rounded-xl border border-forena-100 bg-white px-4 py-3">
      <div class="flex items-center gap-1">
        ...
        <input type="date" v-model="filterDate" class="rounded-md border border-forena-200 bg-white px-2 py-1 text-xs font-semibold tabular-nums text-forena-800 outline-none focus:border-flare-400" />
      </div>
      <div class="flex items-center gap-1.5">
        <span class="text-[11px] font-bold text-forena-400">협력사</span>
        <select v-model="filterPartner" class="rounded-md border border-forena-200 bg-white px-2 py-1 text-xs text-forena-800 outline-none focus:border-flare-400">
      ...
      <div class="flex items-center gap-1.5">
        ...
        <select v-model="filterStatus" class="rounded-md border border-forena-200 bg-white px-2 py-1 text-xs text-forena-800 outline-none focus:border-flare-400">
          <option value="">전체</option>
      ...
      <div class="flex items-center gap-1 rounded-md border border-forena-200 bg-white px-2">
        ...
        <input v-model="searchKeyword" placeholder="위치/지시서 번호/내용" class="w-44 bg-transparent py-1 text-xs text-forena-800 outline-none placeholder:text-forena-300" />
      </div>
    ...
    <div v-if="currentRole === ROLES.MANAGER" class="shrink-0 rounded-xl border border-forena-100 bg-white p-4">
      ...
      <div class="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        <div v-for="g in gateAssignment" :key="g.id" class="rounded-xl border p-3" :class="g.level === 'high' ? 'border-rose-200 bg-rose-50/40' : g.level === 'mid' ? 'border-amber-200 bg-amber-50/40' : 'border-forena-100 bg-forena-50/30'">
          <div class="flex items-start justify-between">
            <div>
              <p class="text-xs font-bold text-forena-900">{{ g.name }}</p>
              <p class="text-[10px] text-forena-500">{{ g.desc }}</p>
            </div>
            <span v-if="g.level === 'high'" class="inline-flex items-center gap-1 rounded-md bg-rose-100 px-1.5 py-0.5 text-[10px] font-bold text-rose-700">
              <AlertTriangle class="h-3 w-3" /> 과밀
            </span>
            <span v-else-if="g.level === 'mid'" class="rounded-md bg-amber-100 px-1.5 py-0.5 text-[10px] font-bold text-amber-700">혼잡</span>
          ...
          <div class="mt-2 flex items-center gap-3 text-[11px] text-forena-700">
            <span class="inline-flex items-center gap-1"><ChevronRight class="h-3 w-3 text-emerald-500" />입차 <b class="tabular-nums">{{ g.in }}</b>대</span>
            <span class="inline-flex items-center gap-1"><ChevronLeft class="h-3 w-3 text-rose-500" />출차 <b class="tabular-nums">{{ g.out }}</b>대</span>
          </div>
          <div class="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-forena-100">
            <div class="h-full rounded-full transition-all" :class="g.level === 'high' ? 'bg-rose-500' : g.level === 'mid' ? 'bg-amber-500' : 'bg-emerald-500'" :style="{ width: Math.min(g.ratio * 100, 100) + '%' }"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex shrink-0 items-center gap-1 overflow-x-auto border-b border-forena-100">
      <button v-for="t in visibleTabs" :key="t.key" @click="filterProcess = t.key" class="shrink-0 border-b-2 px-4 py-2 text-xs font-bold transition" :class="filterProcess === t.key ? 'border-flare-500 text-flare-700' : 'border-transparent text-forena-500 hover:text-forena-700'">
        {{ t.label }}
      </button>
    </div>

    <div class="min-h-0 flex-1 overflow-y-auto">
      <div v-if="!filteredOrders.length" class="flex flex-col items-center justify-center gap-2 py-16 text-center">
        <FileText class="h-8 w-8 text-slate-300" />
        <p class="text-sm text-slate-400">조회된 작업 지시서가 없습니다.</p>
        <button v-if="currentRole === ROLES.WORKER" @click="openCreate" class="mt-1 inline-flex items-center gap-1 rounded-md bg-flare-500 px-3 py-1.5 text-xs font-bold text-white hover:bg-flare-600">
      ...
      <div v-else class="overflow-hidden rounded-xl border border-forena-100 bg-white">
        <table class="w-full text-xs">
          ...
          <tbody class="divide-y divide-forena-100">
            <tr v-for="o in filteredOrders" :key="o.id" class="cursor-pointer hover:bg-forena-50/40" @click="openViewer(o)">
              <td class="px-3 py-3">
                <span class="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-bold ring-1" :class="statusMeta(o.status).cls">
              ...
              <td class="px-3 py-3">
                <p class="flex items-center gap-1 text-forena-800"><MapPin class="h-3 w-3 text-flare-600" />{{ o.location }}</p>
              </td>
              ...
              <td class="px-3 py-3">
                <div v-if="o.equipment.length" class="space-y-0.5">
                  <p v-for="eq in o.equipment" :key="eq.id" class="text-[11px] text-forena-700">
                    ...
                    <span class="ml-1 text-forena-400">({{ gateName(eq.gateIn) }} → {{ gateName(eq.gateOut) }})</span>
                  </p>
                </div>
              ...
              <td class="px-3 py-3 text-right" @click.stop>
                <button @click="openViewer(o)" class="rounded-md border border-forena-200 bg-white p-1.5 text-forena-600 hover:bg-forena-50" title="상세 보기"><Eye class="h-3.5 w-3.5" /></button>
                <button v-if="canEdit(o)" @click="openEdit(o)" class="ml-1 rounded-md border border-flare-200 bg-flare-50 p-1.5 text-flare-700 hover:bg-flare-100" title="수정"><Pencil class="h-3.5 w-3.5" /></button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
      <div v-if="showTaskSelector" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 p-4" @click.self="showTaskSelector = false">
        <div class="flex max-h-[80vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
          <div class="flex shrink-0 items-center justify-between border-b border-forena-100 px-6 py-4">
            <div>
              ...
              <p class="mt-0.5 text-xs text-forena-500">{{ filterDate }} 기준 진행 예정인 작업 목록입니다.</p>
            </div>
            <button @click="showTaskSelector = false" class="text-slate-400 hover:text-forena-700"><X class="h-5 w-5" /></button>
          </div>
          <div class="overflow-y-auto px-6 py-4">
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div v-for="task in availableTasks" :key="task.id" @click="selectTaskForOrder(task)" class="group cursor-pointer rounded-xl border border-forena-200 bg-white p-4 transition-all hover:border-flare-400 hover:bg-flare-50 hover:shadow-md">
                <div class="flex items-start justify-between mb-2">
                  <span class="rounded bg-forena-100 px-1.5 py-0.5 text-[10px] font-bold text-forena-700">{{ task.location }}</span>
                </div>
                <p class="font-bold text-forena-900 group-hover:text-flare-700">{{ task.name }}</p>
                <p class="mt-2 text-xs text-forena-500 line-clamp-2">{{ task.note || '세부 내용 없음' }}</p>
                <div class="mt-3 flex items-center gap-3 text-[11px] font-semibold text-forena-600">
                  <span class="flex items-center gap-1"><Users class="h-3.5 w-3.5" /> {{ task.requiredCount || 0 }}명</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <!-- 기존 지시서 작성 에디터 -->
    <transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
      <div v-if="showEditor && editing" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" @click.self="closeEditor">
        <div class="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
          <div class="flex shrink-0 items-start justify-between border-b border-forena-100 px-6 py-4">
            <div class="flex items-start gap-3">
              <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-flare-50"><ClipboardList class="h-5 w-5 text-flare-600" /></div>
              <div>
                <p class="text-base font-bold text-forena-900">{{ isNew ? '작업 지시서 작성' : '작업 지시서 수정' }}</p>
                <p class="mt-0.5 text-xs text-forena-500"><span class="font-mono text-forena-700">{{ editing.id }}</span> · {{ fmtKor(editing.date) }} · <span class="font-bold text-forena-700">{{ editing.process }} 공정</span></p>
              </div>
            </div>
            <button @click="closeEditor" class="text-slate-400 hover:text-forena-700"><X class="h-5 w-5" /></button>
          </div>
          ...
          <div class="min-h-0 flex-1 overflow-y-auto px-6 py-4">
            <div class="rounded-xl border border-flare-200 bg-flare-50/40 p-4">
              <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p class="mt-0.5 text-[11px] text-forena-600">선택한 세부작업의 위치 · 시간 · 인원 · 장비 · 상세를 자동 입력합니다. 게이트는 직접 지정하세요.</p>
                  </div>
                </div>
                <div class="flex shrink-0 gap-1.5">
                  <button v-if="draftSourcePlan" @click="regenerateDraft" class="inline-flex items-center gap-1 rounded-md border border-flare-300 bg-white px-2.5 py-1 text-[11px] font-bold text-flare-700 hover:bg-flare-50">
              ...
              <div v-if="draftSourcePlan" class="mt-3 rounded-lg bg-white p-3 ring-1 ring-flare-100">
                <p class="text-[10px] font-bold uppercase tracking-wide text-forena-400">원문 (연결된 주간 세부작업)</p>
            ...
            <div class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div>
                ...
                <input type="date" v-model="editing.date" class="w-full rounded-md border border-forena-200 bg-white px-2.5 py-1.5 text-xs outline-none focus:border-flare-400" />
              </div>
              <div>
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500">작업 시간 <span class="text-rose-500">*</span><span v-if="draftAutoFilled.workTime" class="ml-1 rounded bg-flare-100 px-1 text-[9px] font-bold text-flare-700">자동</span></label>
                <input v-model="editing.workTime" placeholder="예: 07:00 ~ 17:00" @input="clearDraftHighlight('workTime')" class="w-full rounded-md border bg-white px-2.5 py-1.5 text-xs outline-none focus:border-flare-400" :class="draftAutoFilled.workTime ? 'border-flare-300' : 'border-forena-200'" />
              </div>
              <div>
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500">공정 명</label>
                <select v-model="editing.process" disabled class="w-full rounded-md border border-forena-200 bg-white px-2.5 py-1.5 text-xs outline-none focus:border-flare-400 disabled:bg-forena-50/40">
              ...
              <div class="sm:col-span-2">
                ...
                <input v-model="editing.partner" placeholder="예: 한울중기" class="w-full rounded-md border border-forena-200 bg-white px-2.5 py-1.5 text-xs outline-none focus:border-flare-400" />
              </div>
              <div>
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500">필요 인원<span v-if="draftAutoFilled.workers" class="ml-1 rounded bg-flare-100 px-1 text-[9px] font-bold text-flare-700">자동</span></label>
                <input type="number" min="0" v-model.number="editing.workers" @input="clearDraftHighlight('workers')" class="w-full rounded-md border bg-white px-2.5 py-1.5 text-xs tabular-nums outline-none focus:border-flare-400" :class="draftAutoFilled.workers ? 'border-flare-300' : 'border-forena-200'" />
              </div>
              <div class="sm:col-span-3">
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500"><MapPin class="mr-0.5 inline h-3 w-3" />작업 위치 <span class="text-rose-500">*</span><span v-if="draftAutoFilled.location" class="ml-1 rounded bg-flare-100 px-1 text-[9px] font-bold text-flare-700">자동</span></label>
                <input v-model="editing.location" placeholder="예: 본동 3층 슬라브" @input="clearDraftHighlight('location')" class="w-full rounded-md border bg-white px-2.5 py-1.5 text-xs outline-none focus:border-flare-400" :class="draftAutoFilled.location ? 'border-flare-300' : 'border-forena-200'" />
              </div>
            </div>
            ...
            <div class="mt-5 rounded-xl border border-forena-100 bg-forena-50/30 p-4">
              <div class="mb-2 flex items-center justify-between">
                <p class="flex items-center gap-1.5 text-[11px] font-bold text-forena-700">
                  <Truck class="h-3.5 w-3.5 text-flare-600" /> 필요 중장비 + 게이트 배정 ({{ editing.equipment.length }})
              ...
              <div class="grid grid-cols-2 gap-2 rounded-lg bg-white p-3 ring-1 ring-forena-100 sm:grid-cols-5">
                <div class="sm:col-span-2">
                  ...
                  <select v-model="editing.equipmentInput.name" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
                    <option value="">장비 선택</option>
                    <optgroup v-for="(items, category) in equipmentList" :key="category" :label="category"><option v-for="equipment in items" :key="`${category}_${equipment}`" :value="equipment">{{ equipment }}</option></optgroup>
                  </select>
                </div>
                <div>
                  <label class="mb-1 block text-[10px] font-bold text-forena-500">대수</label>
                  <input type="number" min="1" v-model.number="editing.equipmentInput.count" class="w-full rounded-md border border-forena-200 px-2 py-1.5 text-xs tabular-nums outline-none focus:border-flare-400" />
                </div>
                <div class="flex-1">
                  <label class="block text-xs font-medium text-slate-500 mb-1">입차 게이트</label>
                  <select v-model="editing.equipmentInput.gateIn" class="w-full h-9 px-3 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-flare-500 outline-none">
                    <option value="" disabled>게이트 선택</option>
                    <option v-for="g in GATES" :key="g.gateIdx" :value="g.gateIdx" :class="g.status === '혼잡' ? 'text-rose-500' : 'text-emerald-500'">{{ getGateLabel(g) }}</option>
                  </select>
                </div>
                <div class="flex-1">
                  <label class="block text-xs font-medium text-slate-500 mb-1">출차 게이트</label>
                  <select v-model="editing.equipmentInput.gateOut" class="w-full h-9 px-3 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-flare-500 outline-none">
                    <option value="" disabled>게이트 선택</option>
                    <option v-for="g in GATES" :key="g.gateIdx" :value="g.gateIdx">{{ getGateLabel(g) }}</option>
                  </select>
                </div>
                <div class="sm:col-span-5">
                  <button @click="addEquipment" class="inline-flex items-center gap-1 rounded-md border border-flare-200 bg-flare-50 px-2.5 py-1 text-[11px] font-bold text-flare-700 hover:bg-flare-100"><Plus class="h-3 w-3" /> 장비 추가</button>
                </div>
              </div>

              <div v-if="editing.equipment.length" class="mt-2 space-y-1.5">
                <div v-for="(eq, i) in editing.equipment" :key="eq.id" class="flex items-center gap-2 rounded-lg bg-white px-3 py-2 ring-1 ring-forena-100">
                  ...
                  <span class="ml-auto flex items-center gap-1.5 text-[11px]">
                    <select v-model="eq.gateIn" class="rounded border border-forena-200 px-1.5 py-0.5 text-[11px] outline-none focus:border-flare-400"><option v-for="g in GATES" :key="g.gateIdx" :value="g.gateIdx">입 · {{ g.gateName }}</option></select>
                    <ArrowRightLeft class="h-3 w-3 text-forena-400" />
                    <select v-model="eq.gateOut" class="rounded border border-forena-200 px-1.5 py-0.5 text-[11px] outline-none focus:border-flare-400"><option v-for="g in GATES" :key="g.gateIdx" :value="g.gateIdx">출 · {{ g.gateName }}</option></select>
                  </span>
                  <button @click="removeEquipment(i)" class="text-slate-400 hover:text-rose-600"><Trash2 class="h-3.5 w-3.5" /></button>
                </div>
              </div>

              <div class="mt-3">
                <p class="mb-1.5 text-[10px] font-bold uppercase tracking-wide text-forena-500">현재 일자 게이트 혼잡도</p>
                <div class="grid grid-cols-2 gap-1.5 sm:grid-cols-4">
                  <div v-for="g in gateAssignment" :key="g.id" class="rounded-md border px-2 py-1.5" :class="g.level === 'high' ? 'border-rose-200 bg-rose-50/40' : g.level === 'mid' ? 'border-amber-200 bg-amber-50/40' : 'border-forena-100 bg-white'">
                    ...
                    <p class="mt-0.5 text-[10px] tabular-nums text-forena-500">입차 {{ g.in }} · 출차 {{ g.out }}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500">작업 상세 내역 <span class="text-rose-500">*</span><span v-if="draftAutoFilled.workDetail" class="ml-1 rounded bg-flare-100 px-1 text-[9px] font-bold text-flare-700">자동</span></label>
                <textarea v-model="editing.workDetail" rows="5" @input="clearDraftHighlight('workDetail')" placeholder="작업 절차, 범위, 협의사항 등" class="w-full resize-none rounded-md border bg-white px-2.5 py-1.5 text-xs leading-relaxed outline-none focus:border-flare-400" :class="draftAutoFilled.workDetail ? 'border-flare-300' : 'border-forena-200'"></textarea>
              </div>
              <div>
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500">안전 유의사항</label>
                <textarea v-model="editing.safety" rows="5" placeholder="추락/낙하/화재 등 위험요인 및 안전조치" class="w-full resize-none rounded-md border border-forena-200 bg-white px-2.5 py-1.5 text-xs leading-relaxed outline-none focus:border-flare-400"></textarea>
              </div>
              <div class="md:col-span-2">
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-forena-500">특이사항</label>
                <textarea v-model="editing.notes" rows="2" placeholder="기상, 선행공정 의존성, 추가 요청 등" class="w-full resize-none rounded-md border border-forena-200 bg-white px-2.5 py-1.5 text-xs leading-relaxed outline-none focus:border-flare-400"></textarea>
              </div>
            </div>

            <div class="mt-5 rounded-xl border border-forena-100 bg-forena-50/30 p-3.5">
              <div class="mb-2 flex items-center justify-between">
                <p class="flex items-center gap-1.5 text-[11px] font-bold text-forena-700"><ImageIcon class="h-3.5 w-3.5 text-flare-600" /> 현장 / 도면 사진 ({{ editing.photos.length }})</p>
                <button @click="pickPhotos" class="inline-flex items-center gap-1 rounded-md border border-flare-200 bg-flare-50 px-2.5 py-1 text-[11px] font-bold text-flare-700 hover:bg-flare-100"><Plus class="h-3 w-3" /> 사진 추가</button>
                <input ref="photoInputRef" type="file" class="sr-only" accept="image/*" multiple @change="onPhotoChange" />
              </div>
              <div v-if="editing.photos.length" class="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
                <div v-for="(photo, i) in editing.photos" :key="photo.id" class="group relative aspect-square overflow-hidden rounded-lg border border-forena-200 bg-white">
                  <img :src="photo.dataUrl" :alt="photo.name" class="h-full w-full object-cover" />
                  <button @click="removePhoto(i)" class="absolute right-1 top-1 rounded-md bg-slate-900/70 p-1 text-white opacity-0 transition group-hover:opacity-100 hover:bg-rose-600"><Trash2 class="h-3 w-3" /></button>
                  <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-900/70 to-transparent px-1.5 py-1"><p class="truncate text-[9px] font-semibold text-white">{{ photo.name }}</p></div>
                </div>
              </div>
            ...
            <div class="mt-3 rounded-xl border border-forena-100 bg-forena-50/30 p-3.5">
              <div class="mb-2 flex items-center justify-between">
                <p class="flex items-center gap-1.5 text-[11px] font-bold text-forena-700"><Paperclip class="h-3.5 w-3.5 text-flare-600" /> 첨부 파일 ({{ editing.files.length }}) <span class="ml-1 text-[10px] font-normal text-forena-400">PDF · Excel · 이미지</span></p>
                <button @click="pickFiles" class="inline-flex items-center gap-1 rounded-md border border-flare-200 bg-flare-50 px-2.5 py-1 text-[11px] font-bold text-flare-700 hover:bg-flare-100"><Plus class="h-3 w-3" /> 파일 추가</button>
                <input ref="fileInputRef" type="file" class="sr-only" accept=".pdf,.xls,.xlsx,.csv,image/*" multiple @change="onFileChangeInput" />
              </div>
              <ul v-if="editing.files.length" class="space-y-1.5">
                <li v-for="(f, i) in editing.files" :key="f.id" class="flex items-center gap-2 rounded-lg bg-white px-2.5 py-2 ring-1 ring-forena-100">
                  ...
                  <button @click="removeFile(i)" class="text-slate-400 hover:text-rose-600"><Trash2 class="h-3.5 w-3.5" /></button>
                </li>
              </ul>
          ...
          <div class="flex shrink-0 items-center justify-between gap-2 border-t border-forena-100 bg-forena-50/40 px-6 py-3">
            ...
            <div class="flex gap-2">
              ...
              <button @click="submitOrder" class="inline-flex items-center gap-1.5 rounded-lg bg-flare-500 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-flare-600"><Send class="h-3.5 w-3.5" /> 제출</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- 뷰어 모달 -->
    <transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
      <div v-if="viewing" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" @click.self="closeViewer">
        <div class="flex max-h-[94vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
          <div class="flex shrink-0 items-center justify-between border-b border-forena-100 px-6 py-4">
            ...
            <div class="flex items-center gap-1">
              <button v-if="canEdit(viewing)" @click="editViewing" class="rounded-md p-1.5 text-forena-500 hover:bg-forena-50 hover:text-forena-800" title="수정"><Pencil class="h-4 w-4" /></button>
              <button @click="closeViewer" class="text-slate-400 hover:text-forena-700"><X class="h-5 w-5" /></button>
            </div>
          </div>
          ...
          <div class="min-h-0 flex-1 overflow-y-auto px-6 py-4">
            ...
            <div class="mt-2 flex flex-wrap gap-1.5">
              <span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-bold ring-1" :class="statusMeta(viewing.status).cls"><component :is="statusMeta(viewing.status).icon" class="h-3 w-3" /> {{ viewing.status }}</span>
              <span class="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-bold text-emerald-700 ring-1 ring-emerald-200">{{ viewing.partner }}</span>
            </div>
            ...
            <div class="mt-4 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
              <div class="rounded-xl border border-forena-100 bg-forena-50/40 px-4 py-3">
                ...
                <p class="mt-1 flex items-center gap-1 text-sm font-bold text-forena-900"><MapPin class="h-3.5 w-3.5 text-flare-600" />{{ viewing.location }}</p>
              </div>
              <div class="rounded-xl border border-forena-100 bg-forena-50/40 px-4 py-3">
              ...
              <div class="rounded-xl border border-forena-100 bg-forena-50/40 px-4 py-3">
                ...
                <p class="mt-1 text-2xl font-bold tabular-nums text-forena-900">{{ viewing.workers }}<span class="text-sm font-normal text-slate-400 ml-1">명</span></p>
              </div>
              <div class="rounded-xl border border-rose-200 bg-rose-50/40 px-4 py-3">
                ...
                <p class="mt-1 text-sm font-bold text-rose-700">{{ viewing.author }}<span class="ml-1 text-[11px] font-normal text-rose-500">{{ viewing.createdAt }}</span></p>
              </div>
            </div>
            ...
            <div class="mt-3 rounded-xl border border-forena-100 px-4 py-3">
              <div class="flex items-center justify-between">
                ...
                <span v-if="viewing.equipment.length" class="text-[10px] font-bold text-emerald-700">배정 완료</span>
              </div>
              <div v-if="viewing.equipment.length" class="mt-2 space-y-1.5">
                <div v-for="eq in viewing.equipment" :key="eq.id" class="flex flex-wrap items-center gap-2 rounded-lg bg-forena-50/60 px-3 py-1.5">
            ...
            <div v-if="viewing.files?.length" class="mt-3 rounded-xl border border-forena-100 px-4 py-3">
              ...
              <ul class="mt-1.5 space-y-1">
                <li v-for="f in viewing.files" :key="f.id" class="flex items-center gap-2 text-xs text-forena-800">
                  <FileText class="h-3.5 w-3.5 text-forena-400" />
                  <span class="flex-1 truncate">{{ f.name }}</span>
                  <span class="text-[10px] text-forena-400 tabular-nums">{{ fmtSize(f.size) }}</span>
                  <button class="rounded p-1 text-forena-400 hover:bg-forena-50 hover:text-forena-700"><Eye class="h-3.5 w-3.5" /></button>
                </li>
              </ul>
            </div>
            ...
            <div v-if="viewing.photos?.length" class="mt-3 rounded-xl border border-forena-100 px-4 py-3">
              ...
              <div class="mt-2 grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
                <div v-for="photo in viewing.photos" :key="photo.id" class="aspect-square overflow-hidden rounded-lg border border-forena-200 bg-white">
                  <img :src="photo.dataUrl" :alt="photo.name" class="h-full w-full object-cover" />
                </div>
              </div>
            </div>
          ...
          <div class="flex shrink-0 items-center justify-end gap-2 border-t border-forena-100 bg-white px-6 py-3">
            <button v-if="currentRole === ROLES.MANAGER && ['승인 대기', '검토 중'].includes(viewing.status)" @click="reject" class="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-white px-4 py-2 text-xs font-bold text-rose-700 hover:bg-rose-50"><AlertTriangle class="h-3.5 w-3.5" /> 반려</button>
            <button v-if="currentRole === ROLES.MANAGER && ['승인 대기', '검토 중'].includes(viewing.status)" @click="approve" class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-500 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-emerald-600"><CheckCircle2 class="h-3.5 w-3.5" /> 승인</button>
            <button @click="closeViewer" class="rounded-lg border border-forena-200 bg-white px-4 py-2 text-xs font-semibold text-forena-700 hover:bg-forena-50">닫기</button>
          </div>
        </div>
      </div>
    </transition>

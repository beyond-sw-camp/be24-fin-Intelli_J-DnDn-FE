...
import { Search, Plus, ShieldCheck, Settings, Pencil, X, Save } from 'lucide-vue-next'

const searchField = ref('아이디')
const searchKeyword = ref('')
const showAccountDrawer = ref(false)
const showRoleDrawer = ref(false)
const editingAdmin = ref(null)

const searchFields = ['아이디', '이름', '권한', '전화번호', '이메일']
...
const roleRules = ref([
  { role: '시스템 관리자', description: '전체 시스템 설정 및 전체 계정 권한을 관리합니다.' },
  { role: '본사 관리자', description: '각각의 현장 데이터 조회가 가능합니다.' },
  {
    role: '현장 관리자',
    description: '현장 운영 데이터 조회와 현장 페이지에 대한 모든 권한을 가집니다.',
  },
])
...
const filteredAdmins = computed(() => {
  ...
  const targetKey = keyMap[searchField.value]
...
const totalCountText = computed(() => `목록(${filteredAdmins.value.length}/${admins.value.length})`)
  ...
  if (editingAdmin.value) {
    admins.value = admins.value.map((admin) =>
      admin.id === editingAdmin.value
        ? { ...admin, ...adminForm.value, updatedAt: now, updatedBy: adminForm.value.userId }
        : admin,
    )
  } else {
    admins.value.unshift({
      ...
      updatedAt: now,
      updatedBy: adminForm.value.userId || 'admin',
      createdAt: now,
    })
  }
  ...
  <div class="space-y-5 pb-10">
      ...
      <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div class="flex items-start gap-4">
          ...
          <div>
            ...
            <h1 class="text-gradient-brand text-2xl font-bold tracking-tight">계정 관리</h1>
            <p class="mt-2 text-sm text-forena-700/80"></p>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
          <div class="flex min-w-0 rounded-xl border border-forena-200 bg-white shadow-sm">
            ...
            >
              <option v-for="field in searchFields" :key="field" :value="field">{{ field }}</option>
            </select>
    ...
    >
      <div class="flex items-center justify-between border-b border-forena-100 px-5 py-4">
        <h2 class="text-sm font-bold text-forena-900">{{ totalCountText }}</h2>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full min-w-[1080px] table-fixed text-left text-sm whitespace-nowrap">
          <thead
          ...
          <tbody class="divide-y divide-forena-50 text-forena-800">
            <tr v-if="filteredAdmins.length === 0">
              <td colspan="11" class="px-6 py-14 text-center text-sm text-slate-400">
